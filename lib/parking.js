...
/**
 * Returns the 10:30pm cutover timestamp (ms) for a given day in Singapore time (UTC+8).
 * Night period: cutover → cutover + 8.5h (7am next day)
 *
 * @param {number} tsMs - any timestamp within the target day (ms)
 * @returns {{ cutoverMs: number, morningMs: number }}
 */
function getNightBoundaries(tsMs) {
  // Work in UTC+8 by shifting the timestamp
  const sgMs = tsMs + 8 * 3600 * 1000;
  const sgDate = new Date(sgMs);

  // Midnight UTC+8 of that day expressed as UTC ms
  const midnightUtcMs =
    Date.UTC(sgDate.getUTCFullYear(), sgDate.getUTCMonth(), sgDate.getUTCDate()) -
    8 * 3600 * 1000;

  // 10:30pm SG = 22h30m = 81000 seconds from midnight
  const cutoverMs = midnightUtcMs + 22 * 3600 * 1000 + 30 * 60 * 1000;
  // 7am next day SG = 8.5h after cutover
  const morningMs = cutoverMs + 8 * 3600 * 1000 + 30 * 60 * 1000;

  return { cutoverMs, morningMs };
}
...
export function processCarparks(rawCarparks, destLat, destLng, durationHours, priority, radiusKm = 2, startTime = Date.now()) {
  ...
  for (const cp of rawCarparks) {
    ...
    const isFreeToday = isFreeDay && isFpsCarpark;
